...
export const DEPARTMENTS: Department[] = [
  {
    ...
    references: [
      { id: 'node-catalog', label: 'Node type catalog (incl. Webflow)', tool: 'workflow_node_types_list' },
    ...
    crud:
      ...
      'WEBHOOK: `workflow_node_add` of a `webhookTrigger` creates its row in the same call and returns `webhook_url` + `trigger_id`; ' +
      'never call `workflow_trigger_create` for that node (409). `data.webhookPath` is only a label: the server assigns ' +
      '`<label>-<random>`, so read `webhook_url` from the response and never build a URL. Webhook auth lives on the row, and ' +
      'a definition write (`workflow_node_update`, `workflow_update`, `workflow_node_add`, an editor save) NEVER changes it ' +
      'on a live URL: the node is left as sent and the write warns `auth_not_applied`. Change it only with ' +
      '`workflow_trigger_update({workflow_id,trigger_id,filter_config:{authentication:"none"}})` (merged into the stored config; ' +
      '"none" = a public form), header auth via `workflow_webhook_auth_set` (name the URL with `trigger_id` or `node_id`; ' +
      'when it cannot pick one row it answers 409 with the candidates and changes nothing), or the editor\'s Apply ' +
      'authentication. No API call issues a bearer token for an existing URL: use Apply authentication, or ' +
      '`workflow_trigger_delete` then `workflow_trigger_create({authentication:"bearer"})`, which gives a NEW URL and ' +
      ...
      '`.unresolved_templates` for every node. NEVER use `workflow_run` to test (it sends real ' +
      'emails/Slack/CRM writes and burns run quota). Debug a real run via `workflow_run_get` (per-node step_states) + `workflow_run_logs`. ' +
      'SCHEDULE: `workflow_set_schedule({workflow_id,cron_expression,timezone?,enabled?})` (upsert), read `workflow_get_schedule`, ' +
      'remove `workflow_delete_schedule`. ROLLBACK: `workflow_versions_list` → `workflow_version_restore` (live webhook URLs ' +
  ...
  },
  {
    ...
    crud:
      ...
      'KB articles: `helpdesk_kb_article_create` / `_update` / `_delete`; list or search with ' +
      '`helpdesk_kb_search` (omit `q` to list the whole KB, `visibility: \'draft\'` for the review ' +
      'queue; `category_id` is optional). Moving visibility to `public` auto-publishes.',
  },
  {
    id: 'pm',
